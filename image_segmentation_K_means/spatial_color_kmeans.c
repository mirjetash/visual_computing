...
int main(int argc, char* argv[])
    ...
    for (int i = 0; i < k; i++)
    {
      ...
      cluster_centers[i].x = rand() % rows;
      cluster_centers[i].y = rand() % cols;
    }
    ...
    while(iterations < no_iter){
      for(i=0; i < rows; i++) // for each data point
        for(j=0; j < cols ; j++){
          ...
          for (int t = 0; t < k; t++){ // for each cluster center
            distance = sqrt(pow(graymap[i*cols+j].red - cluster_centers[t].red, 2)+ pow(graymap[i*cols+j].green - cluster_centers[t].green, 2) +pow(graymap[i*cols+j].blue - cluster_centers[t].blue, 2) +pow(i - cluster_centers[t].x, 2) +pow(j - cluster_centers[t].y, 2));
      ...
      for (int t = 0; t < k; t++){
        ...
        int same_label_posX = 0;
        int same_label_posY = 0;
        int counter = 0;
        for (int i = 0; i < rows; i++){
          for (int j = 0; j < cols; j++){
            if (labels[i*cols+j] == t){
              ...
              same_label_posX = same_label_posX + i;
              same_label_posY = same_label_posY + j;
            }
          }
        }
        if (counter!=0)
        {
          ...
          cluster_centers[t].x = same_label_posX/counter;
          cluster_centers[t].y = same_label_posY/counter;   
        }
      }
      iterations++;
    }
